...
import AvailabilityEditScopeModal from '../components/AvailabilityEditScopeModal';
...
export default function AcademicianAvailabilityPage({
  ...
  const [editScopeSlot, setEditScopeSlot] = useState<AvailabilitySlot | null>(null);
  ...
  return (
    <div className="admin-page animate-fade-in">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-nowrap justify-end gap-2">
                          <AdminActionButton
                            ...
                            icon="edit"
                            onClick={() => {
                              if (slot.recurrenceRuleId != null) {
                                setEditScopeSlot(slot);
                              } else {
                                setEditingSlot(slot);
                              }
                            }}
                          ...
                          <AdminActionButton
                            variant={slot.isBlocked ? 'primary' : 'danger'}
                            icon={slot.isBlocked ? 'check_circle' : 'block'}
                            onClick={() => {
                              setBlockError(null);
                              setBlockTarget(slot);
                            }}
                          >
                            {slot.isBlocked ? 'Kullanıma Aç' : 'Kullanıma Kapat'}
                          </AdminActionButton>
                        </div>
                      </td>
                    </tr>
      ...
      <AvailabilityEditScopeModal
        open={editScopeSlot !== null}
        onClose={() => setEditScopeSlot(null)}
        onSelect={(scope) => {
          const slot = editScopeSlot;
          setEditScopeSlot(null);
          if (!slot) {
            return;
          }
          if (scope === 'series') {
            setEditingRecurrenceSlot(slot);
          } else {
            setEditingSlot(slot);
          }
        }}
